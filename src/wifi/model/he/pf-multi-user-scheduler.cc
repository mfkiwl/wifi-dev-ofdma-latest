...
#include "ns3/log.h"
#include "pf-multi-user-scheduler.h"
...
NS_LOG_COMPONENT_DEFINE ("PfMultiUserScheduler");

NS_OBJECT_ENSURE_REGISTERED (PfMultiUserScheduler);

TypeId
PfMultiUserScheduler::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::PfMultiUserScheduler")
    .SetParent<MultiUserScheduler> ()
    .SetGroupName ("Wifi")
    .AddConstructor<PfMultiUserScheduler> ()
    .AddAttribute ("NStations",
                   "The maximum number of stations that can be granted an RU in a DL MU OFDMA transmission",
                   UintegerValue (4),
                   MakeUintegerAccessor (&PfMultiUserScheduler::m_nStations),
                   MakeUintegerChecker<uint8_t> (1, 74))
    .AddAttribute ("mcs",
                   "The MCS to be assigned to the stations.",
                   UintegerValue (8),
                   MakeUintegerAccessor (&PfMultiUserScheduler::m_mcs),
                   MakeUintegerChecker<uint32_t> (0, 11))               
    .AddAttribute ("EnableTxopSharing",
                   "If enabled, allow A-MPDUs of different TIDs in a DL MU PPDU.",
                   BooleanValue (true),
                   MakeBooleanAccessor (&PfMultiUserScheduler::m_enableTxopSharing),
                   MakeBooleanChecker ())
    .AddAttribute ("ForceDlOfdma",
                   "If enabled, return DL_MU_TX even if no DL MU PPDU could be built.",
                   BooleanValue (false),
                   MakeBooleanAccessor (&PfMultiUserScheduler::m_forceDlOfdma),
                   MakeBooleanChecker ())
    .AddAttribute ("EnableUlOfdma",
                   "If enabled, return UL_MU_TX if DL_MU_TX was returned the previous time.",
                   BooleanValue (true),
                   MakeBooleanAccessor (&PfMultiUserScheduler::m_enableUlOfdma),
                   MakeBooleanChecker ())
    .AddAttribute ("EnableBsrp",
                   "If enabled, send a BSRP Trigger Frame before an UL MU transmission.",
                   BooleanValue (true),
                   MakeBooleanAccessor (&PfMultiUserScheduler::m_enableBsrp),
                   MakeBooleanChecker ())
    .AddAttribute ("UlPsduSize",
                   "The default size in bytes of the solicited PSDU (to be sent in a TB PPDU)",
                   UintegerValue (500),
                   MakeUintegerAccessor (&PfMultiUserScheduler::m_ulPsduSize),
    ...
    .AddAttribute ("UseCentral26TonesRus",
                   ...
                   BooleanValue (false),
                   MakeBooleanAccessor (&PfMultiUserScheduler::m_useCentral26TonesRus),
                   MakeBooleanChecker ())
    .AddAttribute ("MaxCredits",
                   ...
                   TimeValue (Seconds (1)),
                   MakeTimeAccessor (&PfMultiUserScheduler::m_maxCredits),
                   MakeTimeChecker ())
  ;
  return tid;
}

PfMultiUserScheduler::PfMultiUserScheduler ()
  : loopOutput(false), specificOutput(false), m_startStation(0), m_ulTriggerType (TriggerFrameType::BASIC_TRIGGER) 
{
...
void
PfMultiUserScheduler::DoInitialize (void)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT (m_apMac != nullptr);
  m_apMac->TraceConnectWithoutContext ("AssociatedSta",
                                       MakeCallback (&PfMultiUserScheduler::NotifyStationAssociated, this));
  m_apMac->TraceConnectWithoutContext ("DeAssociatedSta",
                                       MakeCallback (&PfMultiUserScheduler::NotifyStationDeassociated, this));
...
void
PfMultiUserScheduler::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
  m_staList.clear ();
  m_candidates.clear ();
...
PfMultiUserScheduler::SelectTxFormat (void)
{
  NS_LOG_FUNCTION (this);
...
MultiUserScheduler::TxFormat
PfMultiUserScheduler::TrySendingBsrpTf (void)
{
  NS_LOG_FUNCTION (this);
...
MultiUserScheduler::TxFormat
PfMultiUserScheduler::TrySendingBasicTf (void)
{
  NS_LOG_FUNCTION (this);
...
void
PfMultiUserScheduler::NotifyStationAssociated (uint16_t aid, Mac48Address address)
{
...
void
PfMultiUserScheduler::NotifyStationDeassociated (uint16_t aid, Mac48Address address)
{
  NS_LOG_FUNCTION (this << aid << address);
...
MultiUserScheduler::TxFormat
PfMultiUserScheduler::TrySendingDlMuPpdu (void)
{
  NS_LOG_FUNCTION (this);
...
//     }
  ...
  std::size_t nCentral26TonesRus;
  // This method call modifies the variable count, because a pointer to it is receiver on the other end
  // It then changes it according to the number of RUs assigned to minimize wastage
  // If count is 6, 26 Tone RUs would result in 3 RUs being wasted, so it assigns 4 RUs and then
  // returns that value to count
  ...
  if ( specificOutput )
    std::cout << m_staList[primaryAc].size() << " stations associated to the AP\n";
  m_staInfo.clear ();
          ...
          if (m_apMac->GetQosTxop (ac)->GetBaAgreementEstablished (staIt->address, tid))
            {
              ...
              NS_LOG_DEBUG ("BA Agreement NOT established with STA " << staIt->aid);
  ...
  if ( specificOutput )
    std::cout << m_staInfo.size() << " stations being considered for RU assignment\n";
...
double PfMultiUserScheduler::getDataRate(int mcs, int ru) {
...
void PfMultiUserScheduler::ProportionalFair(std::vector<int> currRUset,int currMCS){
...
double PfMultiUserScheduler::timeReq(int dataSize,int rusize,int mcs_QAM)
{
  if(dataSize==0)
    return 0;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
double PfMultiUserScheduler::timeReq1(int dataSize,int rusize,int mcs)
{
  if(dataSize==0)
    return 0;
...
void PfMultiUserScheduler::MUTAX(){
  ...
  for (unsigned i=0; i<len; i++) {
    if(bestAssignment[i]!=-1) {
      ...
      if(mapw[startItw->aid]==0){
        mapw[startItw->aid]=st;
      }
    }
...
PfMultiUserScheduler::ComputeDlMuInfo (void)
{
  NS_LOG_FUNCTION (this);
  ...
  if ( specificOutput ) {
    ...
    std::cout<<"nRusAssigned = " << nRusAssigned << "\n";
  }
  ...
  if ( specificOutput ) {
    // Don't uncomment ruType is only assigned if MUTAX doesn't create an RU map, which means that ruType is
    // never assigned
  ...
  // later these stations (not allocated an RU) are removed from the list
  ...
  // time is the first one that was not served this time (This is actually pointless for PF scheduler)
  ...
  // NOTE, this data structure is also used for UL transmission candidates
  ...
  // This credit/Debit system and sorting the station list based on it is
  // unnecessary for a PF scheduler
...
// This method is not used in PF the scheduler, it's replacement method has been
// embedded in the lines after call to this method is commented.
void
PfMultiUserScheduler::AssignRuIndices (WifiTxVector& txVector)
{
...
PfMultiUserScheduler::ComputeUlMuInfo (void)
{
